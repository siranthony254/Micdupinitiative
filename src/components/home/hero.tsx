...
export function HomePage() {
  ...
  const [selectedUpdate, setSelectedUpdate] = useState<SanityUpdate | null>(null);

  useEffect(() => {
    async function fetchRailUpdates() {
      try {
        const response = await fetch('/api/updates/rail');
        const data = await response.json();
        setUpdates(data.updates || []);
      } catch (error) {
        console.error('Failed to fetch rail updates:', error);
      }
    }
  ...
  return (
    <>
      ...
      {/* News Rail */}
      <NewsRail updates={updates} onUpdateClick={setSelectedUpdate} />

      {/* Memo Viewer */}
      <MemoViewer update={selectedUpdate} onClose={() => setSelectedUpdate(null)} />
    </>
  );
}
